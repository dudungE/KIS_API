...
@Service
public class PriceUpRankSerivce {
    // 필드 주입
...
//    ObjectMapper: JSON 응답을 객체로 변환하는 Jackson 라이브러리 컴포넌트
    ...
    @Autowired
    public PriceUpRankSerivce(WebClient.Builder webClientBuilder, ObjectMapper objectMapper) {
    ...
    private Mono<List<PriceUpRankDto>> parseRank(String response) {
        try {
            List<PriceUpRankDto> responseDataList = new ArrayList<>();
            JsonNode rootNode = objectMapper.readTree(response);
            System.out.println("rootNodejjgus29" + rootNode);
            JsonNode outputNode = rootNode.get("output2");
            if (outputNode != null) {
                for (JsonNode node : outputNode) {
                    PriceUpRankDto responseData = new PriceUpRankDto();
    ...
    public Mono<List<PriceUpRankDto>> getPriceUpRank() {

        HttpHeaders headers = createHttpHeaders();

        Mono<List<PriceUpRankDto>> result = webClient.get()
                ...
                .flatMap(response -> parseRank(response));

        System.out.println("jjhebugresult: " + result);
