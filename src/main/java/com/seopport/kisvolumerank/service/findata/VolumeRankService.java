...
//    ObjectMapper: JSON 응답을 객체로 변환하는 Jackson 라이브러리 컴포넌트
    ...
    @Autowired
    public VolumeRankService(WebClient.Builder webClientBuilder, ObjectMapper objectMapper) {
    ...
    private Mono<List<VolumeRankDto>> parseVolumeRank(String response) {
        try {
            List<VolumeRankDto> responseDataList = new ArrayList<>();
            JsonNode rootNode = objectMapper.readTree(response);
            System.out.println("rootNodejjgus29" + rootNode);
            JsonNode outputNode = rootNode.get("output2");
            if (outputNode != null) {
                for (JsonNode node : outputNode) {
                    VolumeRankDto responseData = new VolumeRankDto();
    ...
    public Mono<List<VolumeRankDto>> getVolumeRank() {
        ...
        Mono<List<VolumeRankDto>> result = webClient.get()
                ...
                .flatMap(response -> parseVolumeRank(response));
